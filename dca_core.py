...
def calculate_dca_metrics(df: pd.DataFrame, initial_investment: float, periodic_investment: float, periodicity: str, end_date: date) -> Dict:
    ...
    investments = [initial_investment] + [initial_investment + periodic_investment * i for i in range(1, num_periods)]
    ...
    snapshots = pd.DataFrame({"date": df["date"], "total_investment": investments, "total_value": values, "total_units": units, "price": prices})
