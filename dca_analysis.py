...
def generate_random_date_ranges(df: pd.DataFrame, periodicity: str, 
                              num_tests: int) -> List[Tuple[pd.Timestamp, pd.Timestamp]]:
...
def run_single_test(date_range: Tuple[pd.Timestamp, pd.Timestamp], 
    ...
    return {
        k: metrics[k] for k in metrics if k != "snapshots"
    } | {
        "start_date": date_range[0].strftime("%Y-%m-%d"),
        "end_date": date_range[1].strftime("%Y-%m-%d"),
    }

def run_randomized_tests(asset_data: Dict[str, pd.DataFrame], 
                        params: Dict, num_tests: int) -> Dict[str, Dict]:
    """Run multiple random date range tests in parallel."""
    first_asset_df = resample_price_data(next(iter(asset_data.values())),
                                       params["periodicity"])
    random_ranges = generate_random_date_ranges(first_asset_df, 
                                              params["periodicity"], 
                                              num_tests)
    ...
    with multiprocessing.Pool() as pool:
        for asset, df in asset_data.items():
            metrics = pool.map(
                partial(run_single_test, df=df, params=params),
                random_ranges
            )
            ...
            results[asset] = {
                ...
                for k in metrics[0] if k not in ("start_date", "end_date")
            } | {"all_runs": metrics}
