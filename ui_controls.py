...
from ui_core import create_color_mapping, get_ticker_info, get_ticker_name, initialize_session_state, truncate_name, validate_ticker
...
def display_legend(selected_formatted, name_to_ticker, color_map):
    ...
    for name in selected_formatted:
        ...
        long_name = get_ticker_name(ticker)
        full_name = f"{long_name} ({ticker})" if long_name != ticker else ticker
        truncated_name = f"{truncate_name(long_name)} ({ticker})" if long_name != ticker else ticker
